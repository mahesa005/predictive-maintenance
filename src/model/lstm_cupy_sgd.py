"""
Optimized CuPy LSTM with Batched Operations (SGD Version).
~10-50x faster than naive implementation.
Uses simple SGD optimizer instead of Adam.
...
class LSTMModelGPUOptimizedSGD:
    ...
    def backward_batch(self, y_pred, y_true, caches, cw=1):
        ...
        # Weighted Cross Entropy: Class 1 is weighted more
        weight_1 = cw
        dy = (y_pred - y_true).reshape(1, -1)
        y_true_reshaped = y_true.reshape(1, -1)
        # Weight error for samples with label 1
    ...
    def update_sgd(self, grads, lr=0.01, lambda_l2=0.01):
        """
        Simple SGD update (no momentum, no Adam).
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, 
        ...
        Args:
            ...
            lr: Learning rate (default higher than Adam since no momentum)
        ...
        print(f"Training (SGD): {n_samples} samples, {n_batches} batches/epoch, batch_size={batch_size}")
        if has_validation:
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                # Update (SGD)
                self.update_sgd(grads, lr=lr, lambda_l2=lambda_l2)
